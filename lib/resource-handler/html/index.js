...
HtmlResourceHandler.prototype.loadResourcesForRule = function ($, resource, rule) {
	var self = this;
	var promises = $(rule.selector).map(function loadForElement () {
		...
		return self.handleChildrenPaths(pathContainer, resource, createHtmlData(el.el, el.rule.attr)).then((updatedText) => {
