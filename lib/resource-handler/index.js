 ...
 * @returns {Promise}
 */
ResourceHandler.prototype.handleChildrenResources = function handleChildrenResources (PathHandler, parentResource, text, childResourceHtmlData) {
	var self = this;
	...
	var childrenPromises = childrenPaths.map(function loadChildrenPath (childrenPath) {
		...
		return self.context.requestResource(childResource).then(function updateSrcsetPart (respondedResource) {
