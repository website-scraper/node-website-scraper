...
function loadResourcesForRule (context, resource, rule) {
	...
	var promises = $(rule.selector).map(function loadForElement () {
		...
		if (el.attr(rule.attr)) {
			...
			return context.handleChildrenResources(pathHandler, resource, childResourceHtmlData.attributeValue, childResourceHtmlData).then(function changeAttr (updatedAttr) {
