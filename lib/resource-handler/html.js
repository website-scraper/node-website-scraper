...
var ImgSrcsetTag = require('./resource-path-parsers/html-img-srcset-tag');
var CommonTag = require('./resource-path-parsers/html-common-tag');
...
function loadImgSrcsetResource (context, parentResource, childResourceHtmlData) {
	var imgSrcsetTag = new ImgSrcsetTag(childResourceHtmlData.attributeValue);
	var childrenPaths = imgSrcsetTag.getPaths();
	var pathsToUpdate = [];

	return Promise.mapSeries(childrenPaths, function loadImgSrcsetPart (childrenPath) {
		...
		return context.requestResource(childResource).then(function updateSrcsetPart (respondedResource) {
			if (respondedResource) {
				...
				pathsToUpdate.push({ oldPath: childrenPath, newPath: respondedResource.getFilename()});
		...
		return Promise.resolve(imgSrcsetTag.updateText(pathsToUpdate));
	});
}
...
function loadGeneralResource (context, parentResource, childResourceHtmlData) {
	...
	var commonTag = new CommonTag(attr);
