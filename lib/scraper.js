...
const DefaultSaveResourcePlugin = require('./resource-saver/index').plugin;
const filenamePlugins = require('./filename-generator').plugins;
...
const actionNames = [
	...
	'saveResource', 'generateFilename'
];

const mandatoryActions = [
	{ name: 'saveResource', pluginClass: DefaultSaveResourcePlugin },
	{ name: 'generateFilename', pluginClass: filenamePlugins.byType }
];
...
class Scraper {
	...
	saveResource (resource) {
		...
		return Promise.resolve()
			...
			.then(() => {
				...
				return this.runActions('saveResource', {resource});
			}).then(() => {
	...
	async runActions (actionName, params) {
		...
		for (let action of this.actions[actionName]) {
			if (typeof action === 'function') {
				result = await action({...params, ...result});
			}
		}
		return result;
	}
	...
	async errorCleanup (error) {
		...
		throw error;
		// return this.resourceSaver.errorCleanup(error).then(() => Promise.reject(error));
	}

	scrape (callback) {
		return Promise.bind(this)
			.then(() => this.runActions('beforeStart', {options: this.options}))
			.then(this.load)
