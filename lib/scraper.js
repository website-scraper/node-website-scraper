...
Scraper.prototype.waitForLoad = function waitForLoad (previousCount) {
	var self = this;
	var loadingPromises = _.toArray(self.loadedResourcePromises);
	var count = loadingPromises.length;
	var loadingIsFinished = (count === previousCount);

	if (!loadingIsFinished) {
		return Promise.mapSeries(loadingPromises, function executeLoading (loadingPromise) {
