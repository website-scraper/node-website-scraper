...
function Scraper (options) {
	...
	self.originalResources = self.options.urls.map((obj) => {
		const url = _.isObject(obj) && _.has(obj, 'url') ? obj.url : obj;
		const filename = _.isObject(obj) && _.has(obj, 'filename') ? obj.filename : self.options.defaultFilename;
...
Scraper.prototype.load = function load () {
	var self = this;
	return Promise
		.map(this.originalResources, this.requestResource.bind(this))
...
Scraper.prototype.errorCleanup = function errorCleanup (error) {
	...
	return this.fsAdapter.handleError().then(function loadedDataRemoved () {
