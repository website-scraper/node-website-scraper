...
const { extend, clone, union, urlsEqual, getTypeByMime, getTypeByFilename } = require('./utils');
...
class Scraper {
	constructor (options) {
		...
		this.request = new Request({ httpResponseHandler: this.options.httpResponseHandler });
		this.resourceHandler = new ResourceHandler(this.options, this);
		this.filenameGenerator = new FilenameGenerator(this.options);
		this.resourceSaver = this.options.resourceSaver ? new this.options.resourceSaver(clone(this.options)) : new ResourceSaver(this.options);

		this.resources = this.options.urls.map(({url, filename}) => {
	...
	normalizeOptions (options) {
		...
		this.options.request = typeof options.request === 'function' ? options.request : extend(defaults.request, options.request);

		this.options.urls = Array.isArray(this.options.urls) ? this.options.urls : [this.options.urls];
		this.options.urls.forEach((urlItem, i) => {
			if (typeof urlItem === 'string') {
				this.options.urls[i] = {url: urlItem, filename: this.options.defaultFilename};
			} else {
				this.options.urls[i] = {url: urlItem.url, filename: urlItem.filename || this.options.defaultFilename}
			}
		});
	...
	applyPlugins (plugins = []) {
		...
		plugins.forEach(plugin => plugin.apply(this.addAction.bind(this)));
	}

	addAction (name, handler) {
		if (!actionNames.includes(name)) {
			throw new Error(`Unknown aciton "${name}"`);
		}
	...
	saveResource (resource) {
		...
		return Promise.resolve()
			.then(() => this.resourceHandler.handleResource(resource))
			.then(() => {
				logger.info('saving resource ' + resource + ' to fs');
				return this.resourceSaver.saveResource(resource);
			}).then(() => {
				if (this.options.onResourceSaved) {
					this.options.onResourceSaved(resource);
				}
			}).catch((err) => {
				logger.warn('failed to save resource ' + resource);
				return this.handleError(err, resource);
			});
	}
	...
	createNewRequest (resource) {
		...
		const requestPromise = Promise.resolve()
			.then(() => {
				...
				const opts = typeof this.options.request === 'function' ? this.options.request(resource) : this.options.request;
				return self.requestQueue.add(() => self.request.get(url, referer, opts));
