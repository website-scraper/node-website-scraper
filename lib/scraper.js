...
Scraper.prototype.loadResource = function loadResource (resource) {
	...
	if (loadedResourcePromise) {
		...
		self.addLoadedResourcePromise(url, {
			loaded: 0,
			resource: resource
		});
	}
...
Scraper.prototype.saveResource = function saveResource (resource) {
	var self = this;
	self.addLoadedResourcePromise(resource.getUrl(), {
		loaded: 1,
		resource: resource
	});
...
Scraper.prototype.waitForLoad = function waitForLoad (previousCount) {
	var self = this;
	var loadingUrls = _.filter(self.loadedResourcePromises, function(o) {
		return o.loaded === 0;
	});
	var resources = loadingUrls.map((l) => l.resource);
	var loadingIsFinished = _.isEmpty(loadingUrls);

	if (!loadingIsFinished) {
		return Promise.mapSeries(resources, function(r) {
			return self.saveResource(r);
