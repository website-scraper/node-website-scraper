...
var ScraperCore = function(data){
  ...
  options.staticDirectories = data.hasOwnProperty('staticDirectories') ? data.staticDirectories : config.staticDirectories;
  ...
  return {
    scrape: function(callback) {
      ...
      if(!options.url){
        return callback({status:'error', message:'Url is undefined!'});
      }
      if(!validUrl.isWebUri(options.url)){
        return callback({status:'error', message:'Url ' + options.url + ' is not correct!'});
      }
      if(!options.path){
        return callback({status:'error', message:'Path is undefined!'});
      }
      if(fs.existsSync(options.path)){
        return callback({status:'error', message:'Path ' + options.path + ' exists!'});
      }

      process()
        .then(function(res){return callback(res)})
        .catch(function(e){return callback({status: 'error', message: e.toString()})})
    }
  }
};
