...
class Resource {
	...
	setEncoding (newEncoding) {
		...
		// this is needed for cases when enconding changes for a resource with existing text
		// e.g. http response headers has one encoding but resource content has another encoding so we need to change the text
		if (this.text) {
			this.text = Buffer.from(this.text, this.encoding).toString(newEncoding);
		}
