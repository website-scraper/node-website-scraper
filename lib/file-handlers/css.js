...
function changeExacltyMatchedUrl (text, oldUrl, newUrl) {
	// starts with ' " ( ends with ' " )
	var exactlyMatched = '([\'"\(\s])(' + oldUrl + ')([\'"\)\s])';
	var exactlyMatchedRegexp = new RegExp(exactlyMatched, 'g');
	text = text.replace(exactlyMatchedRegexp, function changeUrl (match, g1, g2, g3) {
		return g1 + newUrl + g3;
	});
	return text;
}
	...
	var promises = _.map(cssUrls, function loadResourceFromCssUrl (cssUrl) {
		...
		return context.loadResource(childResource).then(function handleLoadedSource (loadedResource) {
			if(loadedResource){
				...
				text = changeExacltyMatchedUrl(text, cssUrl, relativePath);
			}
		});
	});
