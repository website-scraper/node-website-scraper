...
function defaultResponseHandler2 ({response}) {
	return Promise.resolve(response.body);
}
...
class Request {
	 ...
	 */
	constructor ({httpResponseHandler} = {}) {
		this.handleResponse = httpResponseHandler || defaultResponseHandler;
	}
	 ...
	 * @return {Promise}
	 */
	get (url, referer, options = {}) {
		let requestOptions = _.clone(options);
...
module.exports.get = function ({url, referer, options = {}, afterResponse = defaultResponseHandler2}) {
