...
var Loader = function (data) {
   ...
   * @param currentOptions {Object} current html options, contain fields: url, filename
   * @returns {string} modified html
   */
  function beforeHtmlProcessing(html, currentOptions) {
    // Mark file as loaded
    setLoadedFilename(currentOptions.url, currentOptions.filename);
    ...
    $('base').each(function () {
      ...
      currentOptions.url = utils.getAbsolutePath(currentOptions.url, href);
    ...
    $('a[href]').each(function () {
      var self = $(this);
      var href = self.attr('href');
      var url = utils.getAbsolutePath(currentOptions.url, href);
  ...
  function start() {
    ...
    return Promise.reduce(options.urlsToLoad, function (result, currentOptions) {
      var p = Promise.resolve();

      // Load html
      p = p.then(function () { return makeRequest(currentOptions.url) })
        .then(function (html) { return beforeHtmlProcessing(html, currentOptions) });

      // Load css
      p = p.then(function (html) {
        return loadCssSources(html, currentOptions.url)
      });
